...
use pdf_core::{Sheet, Viewport, ViewportData, CellRenderData, CellStyle, CellValue};
...
pub fn render_viewport(sheet: &Sheet, vp: &Viewport) -> ViewportData {
    ...
    for row in vp.first_row..last_row {
        for col in vp.first_col..last_col {
            ...
            let merge = sheet.merges.iter().find(|m| m.range.contains_addr(row, col));
...
impl RangeExt for pdf_core::CellRange {
    fn contains_addr(&self, row: u32, col: u32) -> bool {
...
mod tests {
    use super::*;
    use pdf_core::{Sheet, Cell, CellValue};
