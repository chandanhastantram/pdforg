...
use uuid::Uuid;
use pdf_core::document::{Document, Comment};
use pdf_spell::{default_checker, SpellResult};
use pdf_sheets::parser::parse_formula;
use pdf_sheets::evaluator::{eval, EvalContext};
use pdf_core::{Workbook, CellAddress, CellValue};
...
impl From<pdf_storage::StorageError> for ApiError {
    fn from(e: pdf_storage::StorageError) -> Self {
...
pub async fn export_document(
    ...
    match params.format.as_str() {
        "docx" => {
            match pdf_formats::docx::write_docx(&doc) {
                Ok(bytes) => (
                    StatusCode::OK,
        ...
        "pdf" => {
            match pdf_pdf::creator::create_pdf_from_document(&doc) {
                Ok(bytes) => (
                    StatusCode::OK,
        ...
        "xlsx" => {
            ...
            match pdf_formats::xlsx::write_xlsx(&wb) {
                Ok(bytes) => (
                    StatusCode::OK,
...
pub async fn convert_file(mut multipart: Multipart) -> impl IntoResponse {
    ...
    let doc = match ext.as_str() {
        "docx" => pdf_formats::docx::parse_docx(&bytes).ok(),
        "rtf"  => pdf_formats::rtf::parse_rtf(&bytes).ok(),
        "odt"  => pdf_formats::odf::parse_odt(&bytes).ok(),
        _ => None,
    };
