...
async fn cmd_serve(port: u16, data_dir: PathBuf, no_browser: bool) -> Result<()> {
    ...
    pdforg::server::start_server(port, data_dir).await
...
async fn cmd_open(file: PathBuf) -> Result<()> {
    ...
    let doc = match ext.as_str() {
        "docx" => {
            ...
            pdforg::formats::docx::parse_docx(&bytes)
                .context("Failed to parse DOCX")?
        }
        "odt" => {
            let bytes = std::fs::read(&file).context("Failed to read file")?;
            pdforg::formats::odf::parse_odt(&bytes)
                .context("Failed to parse ODT")?
        }
        "rtf" => {
            let bytes = std::fs::read(&file).context("Failed to read file")?;
            pdforg::formats::rtf::parse_rtf(&bytes)
                .context("Failed to parse RTF")?
        }
        "pdf" => {
            let bytes = std::fs::read(&file).context("Failed to read file")?;
            pdforg::storage::Store::import_pdfo(&bytes)
        ...
        "xlsx" => {
            ...
            return pdforg::server::start_server(3847, data_dir).await
                .map_err(|e| anyhow::anyhow!("{}", e));
        }
        "pptx" => {
    ...
    let data_dir = default_data_dir();
    let mut store = pdforg::storage::Store::open(&data_dir)?;
    ...
    pdforg::server::start_server(3847, data_dir).await
        .map_err(|e| anyhow::anyhow!("{}", e))
}
...
fn cmd_convert(input: &PathBuf, output: &PathBuf) -> Result<()> {
    ...
    let doc_result = match in_ext.as_str() {
        "docx" => Some(pdforg::formats::docx::parse_docx(&bytes)?),
        "odt"  => Some(pdforg::formats::odf::parse_odt(&bytes)?),
        "rtf"  => Some(pdforg::formats::rtf::parse_rtf(&bytes)?),
        _ => None,
    };
    ...
    let out_bytes = match out_ext.as_str() {
        "docx" => pdforg::formats::docx::write_docx(&doc)
            .context("Failed to write DOCX")?,
        "pdf"  => pdforg::pdf_tools::creator::create_pdf_from_document(&doc)
...
fn cmd_export(file: &PathBuf, format: ExportFormat, output: Option<&PathBuf>) -> Result<()> {
    ...
    let doc = match ext.as_str() {
        "docx" => pdforg::formats::docx::parse_docx(&bytes)?,
        "odt"  => pdforg::formats::odf::parse_odt(&bytes)?,
        "rtf"  => pdforg::formats::rtf::parse_rtf(&bytes)?,
        _ => anyhow::bail!("Unsupported input format: .{}. Supported: docx, odt, rtf", ext),
    };

    let (out_bytes, out_ext): (Vec<u8>, &str) = match format {
        ExportFormat::Pdf  => (pdforg::pdf_tools::creator::create_pdf_from_document(&doc)?, "pdf"),
        ExportFormat::Docx => (pdforg::formats::docx::write_docx(&doc)?, "docx"),
        ExportFormat::Xlsx => {
            // Export as minimal xlsx from writer content isn't ideal but works for roundtrip
            let wb = pdforg::core::Workbook::default();
            (pdforg::formats::xlsx::write_xlsx(&wb)?, "xlsx")
        }
        ExportFormat::Txt => {
            // Plain text export
            ...
            for block in &doc.body {
                match block {
                    pdforg::core::document::Block::Paragraph(p) => {
                        for run in &p.runs { text.push_str(&run.text); }
                        text.push('\n');
                    }
                    pdforg::core::document::Block::Heading(h) => {
...
fn cmd_spell(file: &PathBuf, lang: &str) -> Result<()> {
    ...
    let checker = pdforg::spell::default_checker();
...
fn extract_text(doc: &pdforg::core::Document) -> String {
    let mut text = String::new();
    for block in &doc.body {
        match block {
            pdforg::core::document::Block::Paragraph(p) => {
...
fn cmd_post_install() -> Result<()> {
    ...
    pdforg::storage::Store::open(&data_dir)?;
