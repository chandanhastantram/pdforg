...
use super::{SharedState, ClientMsg, ServerMsg};
use crate::spell::default_checker;
use crate::sheets::parser::parse_formula;
use crate::sheets::evaluator::{eval, EvalContext};
use crate::core::{Workbook, CellAddress};
use serde_json;
    ...
    match msg {
        ...
        ClientMsg::ApplyOp { doc_id, ops, rev } => {
            ...
            if let Ok(mut doc) = store.load_document(doc_id) {
                ...
                if let Some(crate::core::document::Block::Paragraph(ref mut para)) = doc.body.first_mut() {
                    let original_text: String = para.runs.iter().map(|r| r.text.as_str()).collect::<Vec<_>>().join("");
                    let new_text = crate::writer::ot::apply_to_text(&original_text, &ops);
                    para.runs.clear();
                    para.runs.push(crate::core::document::Run::new(new_text));
                }
                doc.revision = rev + 1;
        ...
        ClientMsg::RequestPage { doc_id, page, scale: _ } => {
            ...
            if let Ok(doc) = store.load_document(doc_id) {
                drop(store);
                let layout_engine = crate::writer::layout::LayoutEngine::new(doc.page_layout.clone());
                let pages = layout_engine.layout_document(&doc);
                if let Some(layout_page) = pages.get(page as usize) {
                    let render = crate::render::rasterizer::render_to_canvas_commands(layout_page);
